...
Both endpoints only support HTTP requests using the POST method. The supported content type for the request is `application/json`.&#x20;
...
The above code block shows how the structure of a typical HTTP request to the Authentication Flow API should be configured.

* The API expects an HTTP **POST** request.
* The endpoint to make an HTTP request to **/api/v1/authentication\_flows**. For example, `https://{your-project.authgear.cloud}/api/v1/authentication_flows`. Replace `{your-project.authgear.cloud}` with the valid domain name for your Authgear project.
...
**Note**: In order to get the `finish_redirect_url` at the end of your authentication flow, you must include the value of the entire URL Query generated by Authgear in your initial HTTP request for your authentication flow. Check the URL Query section of this reference to learn more about what URL Query is.&#x20;
...
* `action.data.finish_redirect_uri` contains a URL that can be used to hand the authentication process back to Authgear so that it can redirect the user back to the client application.
...
The `action.data.options` object in the response contains an array with [all the supported identification methods](authentication-flow-api.md#id-3.0-identifications). For example, the presence of `{ "identification": "email" }` means you can continue the above signup flow using an email address to identify the user.
...
This action creates a new authenticator. For example, you can use the `create_authenticator` action to set a new password for a new user during account creation (signup).
...
From the above response `action.data.options` shows that we can use the `primary_password` authentication method. Learn more about all the authenticators supported by Authgear, in the [Authentications section](authentication-flow-api.md#id-4.0-authentications).
...
`action.data.options` contains a list of all [supported identification methods](authentication-flow-api.md#id-3.0-identifications).
...
* A WebSocket message of a JSON object `{"type": "refresh"}` is sent once the user approves the link. Upon receiving the message, you can [retrieve a state again](authentication-flow-api.md#id-1.5-retrieve-a-state-again).
